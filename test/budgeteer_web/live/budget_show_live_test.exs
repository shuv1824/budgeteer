defmodule BudgeteerWeb.BudgetShowLiveTest do
  ...
  describe "Show budget" do
    ...
    test "redirects to budget list when budget does not exist", %{conn: conn, user: user} do
    ...
    test "redirects to budget list when budget is hidden from the user", %{
      conn: conn,
      budget: budget
    } do
      other_user = Budgeteer.AccountsFixtures.user_fixture()
